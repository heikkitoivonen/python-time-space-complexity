...
| `combinations(iterable, r)` | O(n + r + r×C(n,r)) total | O(n + r) init + O(r) per item | All r-combinations. Setup lands before the first result, so a small result set is not a cheap call. O(r) per result is the upper bound, tight when the results are kept, as `list()` does; consumed one at a time they cost less |
| `combinations_with_replacement(iter, r)` | O(n + r + r×C(n+r-1,r)) total | O(n + r) init + O(r) per item | Combinations allowing repeats; same up-front setup as `combinations()` |
| `permutations(iterable, r)` | O(n + r + r×P(n,r)) total | O(n + r) init + O(r) per item | All permutations |
| `product(iter1, iter2, ...)` | O(k + Σnᵢ + k×n₁×n₂×...×nₖ) total | O(k + Σnᵢ) init + O(k) per item | Cartesian product; every input is stored before the first result, so one empty pool empties the result set without making the call free |
